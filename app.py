...
기존의 고정된 학생 데이터 대신, 선생님이 선택한 **교과, 학년, 학기, 단원에 맞춰** 제미나이(Gemini) API가 **느린/중간/빠른 학습자 프로필과 학습 데이터를 실시간으로 생성**하도록 코드를 수정할 수 있습니다.

이 방식은 훨씬 더 유용합니다. 예를 들어 '3학년 1학기 나눗셈' 단원을 선택하면, AI는 나눗셈 개념 이해에 어려움을 겪는 학생, 구구단은 알지만 나눗셈의 원리를 헷갈려 하는 학생, 심화 문제 풀이가 필요한 학생 등 해당 단원에 특화된 가상 프로필을 만들어주기 때문입니다.

아래는 기존 코드를 수정한 버전입니다. 주요 변경 사항은 다음과 같습니다.

### 주요 변경점

1.  **학생 데이터 동적 생성**:
      * `STUDENT_DATA` 상수를 삭제했습니다.
      * `generate_student_profiles_with_gemini` 함수를 새로 추가했습니다. 이 함수는 선택된 단원 정보를 바탕으로 Gemini API를 호출하여 학생 프로필 3개를 생성합니다.
2.  **`1단계` UI 흐름 변경**:
      * 단원을 선택하면, **[🤖 선택 단원 맞춤 AI 학생 프로필 생성]** 버튼이 나타납니다.
      * 이 버튼을 클릭하면 AI가 학생 프로필을 생성하고, 그 후에 생성된 학생들을 선택할 수 있는 드롭다운 메뉴가 표시됩니다.
3.  **상태 관리**:
      * 생성된 학생 데이터는 `st.session_state.generated_students`에 저장하여, 앱이 재실행되어도 데이터가 유지되도록 했습니다.
      * 4단계에서 학생 정보를 참조할 때도 이 세션 상태(session state)에서 데이터를 가져오도록 수정했습니다.

-----

### 수정된 전체 코드

기존 코드를 아래 코드로 전체 교체하시면 됩니다.
